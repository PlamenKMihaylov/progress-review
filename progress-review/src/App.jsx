...
function App() {
  ...
  useEffect(() => {
    ...
    const initDeck = async () => {
      try {
        ...
        if (!isActive) return
        setStatus('Failed to initialize the deck. Please refresh.')
      } finally {
  ...
  }, [currentCard, deckId, isDrawing, remaining])
...
const CardDisplay = memo(function CardDisplay({ card, isPlaceholder }) {
  const altText = isPlaceholder
    ? 'No card available'
    : `${card.value} of ${card.suit}`

  return (
    <div className={`card-display ${isPlaceholder ? 'placeholder' : ''}`}>
      {card.image ? (
